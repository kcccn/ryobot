...
- **只保留仓库本地必须声明的东西**：触发条件、定时器和权限
- **把运行逻辑全部外包给 ryobot**：安装、默认配置、并发控制、单 bot 路由、街溜子门禁都在可复用 workflow 内部
- **通过 `@main` 自动吃到新版本**：外部仓库通常不需要频繁改 YAML

关键要点：
- **无需 checkout、无需 pip install、无需 matrix strategy、无需本地并发配置**：这些都封装在 ryobot 可复用 workflow 内部
- **版本自动跟随**：`@main` 始终使用最新版；如果你更看重稳定性，可以改成固定 commit SHA
- **无需 Fork**：你不需要复制任何 Python 文件到你的仓库
- **`uses:` 引用自动在 caller 仓库上下文中运行**：bot 读写的 Issue、PR、标签都作用于你的仓库
- **仓库级物理互斥**：可复用 workflow 内部已经带了 repo 级 `concurrency`
- **默认零配置可跑**：不传 `with:` 也能工作；只有你要覆盖默认值时才需要额外配置
...
- 确认 bot 不在冷却窗口内：默认被动事件是 `480-720s`，街溜子模式是 `60-180s`
...
- **仓库级疲劳**：bot 的 `last_spoke_at` / `next_available_at` 写入 coordination issue，跨线程生效；被动事件和街溜子模式分别使用不同冷却窗
...
以下环境变量由 `ryobot` 命令读取。对于外部仓库，推荐优先使用 **Settings → Secrets and variables → Actions → Variables** 中的同名 repo variable 来覆盖默认值，而不是频繁修改 workflow 文件。

优先级如下：

1. caller workflow 的 `with:`
2. caller 仓库或组织的同名 `Actions Variable`
3. `ryobot` 可复用 workflow 内置默认值
...
| `RYOBOT_STREET_LURKER_FATIGUE_MIN_SECONDS` | `60` | 街溜子模式实际行动后的最短疲劳时间 |
| `RYOBOT_STREET_LURKER_FATIGUE_MAX_SECONDS` | `180` | 街溜子模式实际行动后的最长疲劳时间 |
| `RYOBOT_INITIAL_HISTORY_COMMENT_LIMIT` | `12` | 初始只加载最近多少条评论进入上下文 |
| `RYOBOT_INITIAL_HISTORY_TOTAL_CHARS` | `16000` | 初始评论切片的字符预算 |
| `RYOBOT_ALLOWED_WORKFLOWS` | 空 | 允许 DispatchWorkflow skill 触发的 workflow 文件名列表，逗号分隔；为空则不启用该 skill |
...
- workflow 文件长期保持上面的极简 wrapper 不动
- 需要调参时，优先在仓库 Variables 里加同名变量
- 只有做临时覆盖或特殊仓库定制时，才在 `with:` 里显式传参

例如，如果你想启用 `dispatch_workflow` 并调整阈值，可以直接在仓库 Variables 里设置：

- `RYOBOT_ALLOWED_WORKFLOWS=ryobot.yml`
- `RYOBOT_MOTIVATION_THRESHOLD=75`
- `RYOBOT_FATIGUE_MIN_SECONDS=600`
- `RYOBOT_FATIGUE_MAX_SECONDS=900`
- `RYOBOT_STREET_LURKER_FATIGUE_MIN_SECONDS=60`
- `RYOBOT_STREET_LURKER_FATIGUE_MAX_SECONDS=180`

如果你确实想在 YAML 里显式覆盖，常用变量（`MAX_ITERATIONS`、`RYOBOT_MOTIVATION_THRESHOLD`、`RYOBOT_FATIGUE_MIN_SECONDS`、`RYOBOT_FATIGUE_MAX_SECONDS`、`RYOBOT_STREET_LURKER_FATIGUE_MIN_SECONDS`、`RYOBOT_STREET_LURKER_FATIGUE_MAX_SECONDS`、`RYOBOT_INITIAL_HISTORY_COMMENT_LIMIT`、`RYOBOT_ALLOWED_WORKFLOWS`）仍然暴露为 `with:` 输入。例如：
...
jobs:
  ryobot:
    ...
    with:
      ...
      street_lurker_fatigue_min_seconds: 60
      street_lurker_fatigue_max_seconds: 180
      allowed_workflows: ryobot.yml
```
...
只有 `will_reply=true` 且 `motivation_score >= RYOBOT_MOTIVATION_THRESHOLD` 时才会进入第二阶段真正回帖或直接动手。动作完成后，仓库会把该 bot 的疲劳信息写入 coordination issue，因此冷却是全仓库共享的，而不是某条线程单独计时。默认被动事件使用 `RYOBOT_FATIGUE_*`，街溜子模式使用更轻的 `RYOBOT_STREET_LURKER_FATIGUE_*`。
