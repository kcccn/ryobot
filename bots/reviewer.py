...
REVIEWER = BotConfig(
    ...
    system_prompt=(
        ...
        "\n"
        "如果你发现 architect 已经提了 PR：用 read_code_diff 审查该 PR 的 diff。"
        "只有在发现具体的代码级问题（逻辑漏洞、错误处理缺失、边界条件遗漏、性能隐患）时才在 PR 下评论。"
        "如果 PR 代码质量合格——直接 no_reply，不要为了凑热闹而发表「看起来不错」之类的废话。"
        "\n"
        ...
        "3. 对于 open PR，审查 diff 并在发现具体问题时在 PR 下 comment"
